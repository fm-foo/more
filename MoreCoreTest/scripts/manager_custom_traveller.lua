...
-- MoreCore v0.60 register "trav"
function onInit()
  CustomDiceManager.add_roll_type("trav", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /trav [2d6+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "trav", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
