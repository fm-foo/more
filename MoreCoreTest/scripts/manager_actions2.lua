...
function performAction(draginfo, rActor, rRoll, rTarget)
	if not rRoll then
		return;
	end
	
	performMultiAction(draginfo, rActor, rTarget, rRoll.sType, { rRoll });
end

function performMultiAction(draginfo, rActor, rTarget, sType, rRolls)
	if not rRolls or #rRolls < 1 then
		return false;
	end
	
	if draginfo then
		encodeActionForDrag(draginfo, rActor, rTarget, sType, rRolls);
	else
		actionDirect(rActor, rTarget, sType, rRolls);
	end
	return true;
end
...
function createActionMessage(rSource, rRoll, rTarget)
