...
--- rRoll.sUser = User.getUsername();
  ...
  local sDice, sMulti, sDesc = string.match(sParams, "([^%s^x]+)x(%d+)%s*(.*)");
  --Debug.chat("Dice: ", sDice, sMulti, SDesc);
  ...
  if #aNonStandardResults > 0 then
	...
	local rRoll = { sType = sCmd, sDesc = sDesc, aDice = aFinalDice, nMod = nMod, nMulti = tonumber(sMulti) };
	return(rRoll);
...
nTotal = 0;
  ...
  rRoll.aTotal = nTotal * rRoll.nMulti;
...
function onDiceTotal( messagedata )
  ...
  local nMulti = tonumber (string.match(messagedata.text, "Damage\]%s(%d+)"));
  --Debug.chat("nMulti: ", nMulti);
  return true, nMulti;
end
...
--	Debug.chat("nDamage: ", rRoll.aTotal);
  ...
  rMessage.dicedisplay = 0; -- do not display total
...
function applyDamage(rTarget, nDamage)
  ...
  if rTarget ~= nil and rTarget.sCTNode ~= nil then
    ...
    Debug.chat("myTarget: ", myTarget, myTargetNewWounds);
...
--    Debug.chat("sTargetType, sTargetNode: ", sTargetType, sTargetNode);
