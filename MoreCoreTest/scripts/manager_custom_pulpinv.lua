...
-- MoreCore v0.60 register "pulpinv"
function onInit()
  CustomDiceManager.add_roll_type("pulpinv", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /pulpinv [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "pulpinv", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
