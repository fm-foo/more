...
-- MoreCore v0.60 register "branged"
function onInit()
  CustomDiceManager.add_roll_type("branged", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /branged [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "branged", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
