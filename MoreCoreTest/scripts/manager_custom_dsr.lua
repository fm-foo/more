...
-- MoreCore v0.60 register "dsr"
function onInit()
  CustomDiceManager.add_roll_type("dsr", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /dsr [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "dsr", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
