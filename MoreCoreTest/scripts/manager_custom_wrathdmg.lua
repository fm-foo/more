...
-- MoreCore v0.60 register "wrathdmg"
local sCmd = "wrathdmg";
...
function orderDiceResults(rRoll)
  -- Sort rRoll.aDice table based off a.result (the dice result)
   ...
   for _,v in ipairs(rRoll.aDice) do
	...
	if rRoll.aDice[x].result < 4 then 
		nTotal = nTotal + 0;
	Debug.console("nTotal: 123 ", nTotal, x);
		elseif rRoll.aDice[x].result == 6 then 
		nTotal = nTotal + 2;
	Debug.console("nTotal: 6 ", nTotal, x);
		elseif rRoll.aDice[x].result > 3 and rRoll.aDice[x].result <6 then
		nTotal = nTotal + 1;
