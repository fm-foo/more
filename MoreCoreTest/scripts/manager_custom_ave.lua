...
-- MoreCore v0.60 register "ave"
function onInit()
  CustomDiceManager.add_roll_type("ave", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /ave [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "ave", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
