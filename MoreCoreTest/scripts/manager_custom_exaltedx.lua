...
-- MoreCore v3.0 register "exaltedx"
local sCmd = "exaltedx";
...
function createRoll(sParams)
  ...
  if not sParams:match("(%d+)d([%dF]*)x(%d+)%s(.*)") then
	...
	local sNum, sSize, nBonusSuccess, sDesc = sParams:match("(%d+)d([%dF]+)x(%d+)%s*(.*)");
	rRoll.sDesc = sDesc;
  ...
  rRoll.sNum = sNum;
  rRoll.sSize = sSize;
  rRoll.nBonusSuccess = nBonusSuccess;
...
function countSuccess(rRoll)
  ...
  -- Sort rRoll.aDice table based off a.result (the dice result)
	...
	rRoll.nSuccess = nSuccess + rRoll.nBonusSuccess;
...
function createHelpMessage()  
  ...
  rMessage.type = sCmd;
  rMessage.text = rMessage.text .. "The \"/exaltedx #d10\" command is used to roll a set of d10. 7s and above are success. If you obtain no successes and at least one 1s, it's a botch!\n"; 
