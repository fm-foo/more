...
-- MoreCore v0.60 register "wluck"
function onInit()
  CustomDiceManager.add_roll_type("wluck", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /wluck [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "wluck", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
...
Debug.console("onLanded: ", rSource, rTarget, rRoll);
  ...
  Debug.console("nTotal2: ", nTotal)
	...
	local nLuck = DB.getValue(nodeChar .. ".five", 0)-1;
	DB.setValue(nodeChar .. ".five", "number", nLuck);
