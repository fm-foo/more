...
nTotal = 0;
  ...
  local save = tonumber(rRoll.nSave);
	...
	if nTotal > save then
		...
		sSaveResult = "has passed on from this realm";
		

	elseif nTotal <= save then
		Debug.console("Success: ", nTotal);
		sSaveResult = "barely clings on to life";
		end

	if nTotal == 8 or nTotal == 11 or nTotal == 18 then
		sSaveMsg = "With an ";
	else sSaveMsg = "With a ";
		end
...
--	Debug.chat("rSource: ", rSource, rSource.sName);
  
    rMessage.text = rMessage.text .. "\n[Death Save - Target " .. rRoll.aSave .. "]\n" .. sSaveMsg .. "[" .. rRoll.aTotal .. "] " .. rSource.sName .. " " .. rRoll.aSaveResult .. ".";
