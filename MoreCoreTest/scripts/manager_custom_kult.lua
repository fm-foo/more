...
-- MoreCore v0.60 register "kult"
function onInit()
  CustomDiceManager.add_roll_type("kult", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /kult [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "kult", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
...
Debug.console("onLanded: ", rSource, rTarget, rRoll);
  ...
  elseif nTotal >= 10 then
    rMessage.text = "\n" .. rMessage.text .. "\n[Success with Complication] ".. nTotal;
  else
    rMessage.text = "\n" .. rMessage.text .. "\n[Failure] ".. nTotal;
  end
  if rTarget then
