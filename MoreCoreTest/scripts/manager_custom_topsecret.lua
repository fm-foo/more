...
--	Debug.console("Preflightcheck: ", rRoll.sDesc);
  ...
  local sDice, sOp, sSave1, sSave2 = sDicePattern:match("([^%s]+)([<>])(%d+)+(%d+)");
--	Debug.chat("sDice, sOp, sSave1, sSave2: ", sDice, sOp, sSave1, sSave2);
  local nSave1 = tonumber(sSave1);
  local nSave2 = tonumber(sSave2);
	local sDesc1, nMod = ModifierStack.getStack(true);  
	local nSave = math.floor(((nSave1+nSave2)/2)+nMod);
