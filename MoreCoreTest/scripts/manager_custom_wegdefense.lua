...
Debug.console("performAction: rRoll ", rRoll, " rActor: ", rActor);
...
function onLanded(rSource, rTarget, rRoll, rActor)
--  Debug.console("onLanded:wegdefense:49: ", rSource, rTarget, rRoll, " rActor: ", rActor);
	local sDesc, nMod = ModifierStack.getStack(true);
	rNode = ActorManager.getCreatureNode(rSource);
--	Debug.chat("rNode:52:wegdefense: ", rNode);
    rRoll.vNode = ActorManager.getCreatureNode(rSource);
--	Debug.chat("rRoll.vNode:54:wegdefense: ", rRoll.vNode);
--	Debug.chat("rRoll:55:wegdefense: ", rRoll);

    rRoll.aNode = ActorManager.getCreatureNode(rSource);
--	Debug.chat("rRoll.aNode:54:wegdefense: ", rRoll.aNode);
...
--	Debug.console("Result 3: ", rRoll.aDice[3].result);
	...
	if rRoll.aDice[1].result > 1 then
	  ...
	  for i,v in ipairs (rRoll.aDice) do
		...
		rNode.getChild("defence").setValue(nWEGResult);
