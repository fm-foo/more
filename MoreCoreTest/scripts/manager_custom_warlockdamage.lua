...
--	Debug.chat("DR!!!!");
	...
	myTargetHP = tonumber(DB.getValue(rTarget.sCTNode .. ".health"));
	myTargetWounds = tonumber(DB.getValue(rTarget.sCTNode .. ".wounds"));
--	Debug.chat(myTargetArmor, myTargetHP, myTargetWounds);
	if myTargetHP+myTargetArmor-myTargetWounds < rRoll.aTotal then
		rMessage.text = rMessage.text .. "\nCritical Hit - roll for effects";
	end
	local nMinDamage = rRoll.aTotal - myTargetArmor;
--	Debug.chat("nMinDamage!!!!");
	if nMinDamage < 1 then nMinDamage = 1; end
