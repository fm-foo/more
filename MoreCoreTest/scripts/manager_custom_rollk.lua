...
--- rRoll.sUser = User.getUsername();
  ...
  if(not sParams:match("(%d+)d([%dF]*)k(%d+)%s*(.*)") and sParams:match("(%d+)d([%dF]+)%s*(.*)")) then
    sDicePattern = sDicePattern .. "k0"
  end
  
  -- Now we check that we have a properly formatted parameter, or we set the sDesc for the roll with a message.
  if not sDicePattern:match("(%d+)d([%dF]*)k(%d+)") then
    rRoll.sDesc = "Parameters not in correct format. Should be in the format of \"#d#k#\"";
    return rRoll;
  end

  local sNum, sSize, sKeep = sDicePattern:match("(%d+)d([%dF]+)k(%d+)");
  local count = tonumber(sNum);
  local keep = tonumber(sKeep);

  if (keep > count) then
    rRoll.sDesc = "You cannot keep more results than the number of dice being rolled.";
    return rRoll;
  end
