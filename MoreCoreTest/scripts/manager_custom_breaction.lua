...
-- MoreCore v0.60 register "breaction"
function onInit()
  CustomDiceManager.add_roll_type("breaction", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /breaction [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "breaction", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
