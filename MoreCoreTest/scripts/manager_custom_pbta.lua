...
-- MoreCore v0.60 register "pbta"
function onInit()
  CustomDiceManager.add_roll_type("pbta", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /pbta [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
...
Debug.console("onLanded: ", rSource, rTarget, rRoll);
  ...
  elseif nTotal >= 7 then
    rMessage.text = "\n" .. rMessage.text .. "\n[Success with Cost] ".. nTotal;
  else
    rMessage.text = "\n" .. rMessage.text .. "\n[Failure with Hard Choice] ".. nTotal;
  end
  if rTarget then
