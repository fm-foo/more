...
function onInit()
-- Debug.chat("onInit");
  CustomDiceManager.add_roll_type("wrathe", performAction, onLanded, true, "all");
end
...
--		for _,v in ipairs(rRoll.aDice) do
		if (rRoll.aDice[cnt].result < 4) then
			nFailure = nFailure + 1;
			elseif (rRoll.aDice[cnt].result > 3) and (rRoll.aDice[cnt].result < 6)then
			nSuccess = nSuccess + 1;
			elseif (rRoll.aDice[cnt].result == 6) then
...
--Debug.chat("createChatMessage");
  ...
  rMessage.text = rMessage.text .. " (" .. rRoll.sNum .. "d" .. rRoll.sSize .. ")" .. rRoll.sModDesc .. "\rFails: " .. rRoll.nFailure .. ", Successes: " .. rRoll.nSuccess .. ", Epic Success: " .. rRoll.nEpicSuccess;
