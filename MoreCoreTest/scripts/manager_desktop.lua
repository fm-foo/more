...
function registerStackShortcuts(aShortcuts)
	...
	for _,v in ipairs(aShortcuts) do
		registerStackShortcutMC(v.icon, v.icon_down, v.tooltipres, v.class, v.path);
	end
	bDelayUpdate = false;
	updateControls();
end
...
function registerStackShortcutMC(iconNormal, iconPressed, sTooltipRes, className, recordName, bFront)
	function fCreate()
