...
--- rRoll.sUser = User.getUsername();
  ...
  local rRoll = { sType = sCmd, sDesc = sDesc, aDice = aFinalDice, nMod = nMod, nnCrit = tonumber(sCrit) };
...
nTotal = 0;
  ...
  local nCrit = tonumber(rRoll.nnCrit);
	Debug.console("nCrit (nCrit): ", nCrit);
	Debug.console("nCrit (sCrit): ", rRoll.sCrit);
	Debug.console("nCrit (nnCrit): ", rRoll.nnCrit);
	Debug.console("sDice: ", sDice);
  ...
  rRoll.aTotal = nTotal;
  rRoll.aCrit = nCrit;
  rRoll.aDieRoll = nDieRoll;
  rRoll.aScritResult = sCritResult;
  return rRoll;
end
---
...
function createChatMessage(rSource, rRoll)  
  local rMessage = ActionsManager.createActionMessage(rSource, rRoll);
    ...
    rMessage.text = rMessage.text .. "\n" .. rRoll.aScritResult .. " [" .. rRoll.aTotal .. "]";
