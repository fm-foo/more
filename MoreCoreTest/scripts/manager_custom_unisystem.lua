...
--	Debug.console("nMyTotal 1st", nMyTotal);
  ...
  for i , v in ipairs (aSavedDice) do
    ...
    if v.exploded then
      ...
      if w.result == 10 and v.type == "d10" then
        aRerollDice[j] = "d6";
        j = j + 1;
        v.exploded = true;
      ...
      elseif w.result == 1 and v.type == "d10" and w.type ~= "d6" then 
        aRerollDice[j] = "d6";
        j = j + 1;
        v.exploded = true;
		rRoll.sSuccess = "Fail 1";
		Debug.console("Fail 1: ", rRoll.sSuccess);
			if not sDiceLanded then
				sDiceLanded = w.result;
			else
				sDiceLanded = sDiceLanded .. " " .. w.result;
      ...
      elseif w.result == 6 and w.type == "d6" and sResult == "Fail" then
        aRerollDice[j] = "d6";
        j = j + 1;
        v.exploded = true;
		rRoll.sSuccess = "Fail 6";
		Debug.console("Fail 6: ", rRoll.sSuccess);
			if not sDiceLanded then
				sDiceLanded = w.result;
			else
				sDiceLanded = sDiceLanded .. " " .. w.result;
		...
		nNewValue = 1 - w.result;
		Debug.console("nNewValue", nNewValue);
		Debug.console("nMyTotal", nMyTotal);
		nMyTotal = nMyTotal + nNewValue;
		Debug.console("nMyTotal", nMyTotal);
		
      elseif w.result <= 5 and w.type == "d6" and sResult == "Fail" then
        v.exploded = false;
		rRoll.sSuccess = "Fail < 6";
		Debug.console("Fail < 6: ", rRoll.sSuccess);
			if not sDiceLanded then
