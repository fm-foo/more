...
-- MoreCore v0.60 register "talislanta"
function onInit()
  CustomDiceManager.add_roll_type("talislanta", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /talislanta [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "talislanta", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
