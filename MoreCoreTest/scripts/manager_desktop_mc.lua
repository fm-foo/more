...
local m_nMaxDockCols = 1;
local m_dMinDockScale = 0.75;

local m_rcDockOffset = { left = 2, top = 5, right = 2, bottom = 0 };
local m_szDockButton = { w = 100, h = 34 };
local m_szDockSpacing = { w = 0, h = 2 };
local m_rcDockFrameOffset = { left = 5, top = 5, right = 5, bottom = 5 }
...
aDefaultSidebarState = {
	["gm"] = "charsheet,note,image,table,story,quest,npc,battle,item,treasureparcel,vehicle",
	["play"] = "charsheet,note,image,story,npc,item",
	["create"] = "charsheet",
};
...
function onInit()
	Comm.registerSlashHandler("sidebar", processSidebar);
...
function processSidebar(sCommand, sParams)
	local sMode = StringManager.trim(sParams);
	DesktopManager.resetSidebar(sMode);
end
...
function setDockIconSizeAndSpacing(iw, ih, sw, sh)
	Debug.console("DesktopManager.setDockIconSizeAndSpacing: Deprecated on 2021-02-01")
end

function setMinDockScaling(dScale)
	Debug.console("DesktopManager.setMinDockScaling: Deprecated on 2021-02-01")
end

function setMaxDockColumns(nMaxDockCols)
	Debug.console("DesktopManager.setMaxDockColumns: Deprecated on 2021-02-01")
end

function showDockTitleText(bState)
	Debug.console("DesktopManager.showDockTitleText: Deprecated on 2021-02-01")
end

function setDockTitleFont(sFont)
	Debug.console("DesktopManager.setDockTitleFont: Deprecated on 2021-02-01")
end

function setDockTitleFrame(sFrame, nLeft, nTop, nRight, nBottom)
	Debug.console("DesktopManager.setDockTitleFrame: Deprecated on 2021-02-01")
end

function setDockTitlePosition(sAnchor, nX, nY)
	Debug.console("DesktopManager.setDockTitlePosition: Deprecated on 2021-02-01")
end
...
function updateControls()
	...
	local rAreas = DesktopManager.calcAreas(szWindow);
	...
	DesktopManager.layoutArea(m_aStackControls, m_rcStackOffset, m_szStackIcon, nil, m_szStackSpacing, rAreas.nStackCols, rAreas.szStack, 1, szBar);

	DesktopManager.layoutArea(m_aDockControls, m_rcDockOffset, m_szDockButton, m_rcDockFrameOffset, m_szDockSpacing, rAreas.nDockCols, rAreas.szUpperDock, rAreas.dDockScale, szBar);

	szBar.h = szWindow.h - rAreas.szLowerDock.h - (m_rcLowerDockOffset.top + m_rcLowerDockOffset.bottom);
	DesktopManager.layoutArea(m_aLowerDockControls, m_rcLowerDockOffset, m_szDockButton, m_rcDockFrameOffset, m_szDockSpacing, rAreas.nDockCols, rAreas.szLowerDock, rAreas.dDockScale, szBar);
...
function calcAreas(szWindow)
	...
	rAreas.szUpperDock.w = (rAreas.nDockCols * m_szDockButton.w) + ((rAreas.nDockCols - 1) * m_szDockSpacing.w) + (m_rcDockOffset.left + m_rcDockOffset.right);
	...
	rAreas.szStack.h = DesktopManager.calcSectionHeight(m_aStackControls, rAreas.nStackCols, m_szStackIcon, m_szStackSpacing);

	rAreas.szUpperDock.h = DesktopManager.calcSectionHeight(m_aDockControls, rAreas.nDockCols, m_szDockButton, m_szDockSpacing);
	rAreas.szLowerDock.h = DesktopManager.calcSectionHeight(m_aLowerDockControls, rAreas.nDockCols, m_szDockButton, m_szDockSpacing);
	...
	if (nRemainderH > 0) and (nTotalDockH > nRemainderH) then
		rAreas.dDockScale = math.max(nRemainderH / nTotalDockH, m_dMinDockScale);
...
function layoutArea(aControls, rcOffset, szIcon, rcFrameOffset, szSpacing, nCols, szArea, dScale, szBar)
	...
	for i = 1, nFullRows do
		for j = 1, nCols do
			...
			layoutControl(aControls[nIndex], szPlacement, szScaledIcon, rcFrameOffset);
...
function layoutControl(c, szPlacement, szIcon, rcFrameOffset)
	...
	if rcFrameOffset then
		tBounds.left = tBounds.left + rcFrameOffset.left;
		tBounds.top = tBounds.top + rcFrameOffset.top;
		tBounds.width = math.max(tBounds.width - rcFrameOffset.left - rcFrameOffset.right, 0);
		tBounds.height = math.max(tBounds.height - rcFrameOffset.top - rcFrameOffset.bottom, 0);
	end
	c.setStaticBounds(tBounds.left, tBounds.top, tBounds.width, tBounds.height);
end

function registerStackShortcuts(aShortcuts)
	bDelayUpdate = true;
	for _,v in ipairs(aShortcuts) do
		DesktopManager.registerStackShortcut2(v.icon, v.icon_down, v.tooltipres, v.class, v.path);
	end
	bDelayUpdate = false;
	DesktopManager.updateControls();
end
...
function registerStackShortcut(iconNormal, iconPressed, tooltipText, className, recordName)
	function fCreate()
		DesktopManager.createStackShortcut(iconNormal, iconPressed, tooltipText, className, recordName);
	end
...
function registerStackShortcut2(iconNormal, iconPressed, sTooltipRes, className, recordName, bFront)
	function fCreate()
		DesktopManager.createStackShortcut(iconNormal, iconPressed, Interface.getString(sTooltipRes), className, recordName, bFront);
	end
...
function registerDockShortcuts(aShortcuts)
	bDelayUpdate = true;
	for _,v in ipairs(aShortcuts) do
		DesktopManager.registerDockShortcut2(v.icon, v.icon_down, v.tooltipres, v.class, v.path, v.subdock);
	end
	bDelayUpdate = false;
	DesktopManager.updateControls();
end
...
function registerDockShortcut(iconNormal, iconPressed, tooltipText, className, recordName, useSubdock)
	function fCreate()
		DesktopManager.createDockShortcut(iconNormal, iconPressed, tooltipText, className, recordName, useSubdock);
	end
...
function registerDockShortcut2(iconNormal, iconPressed, sTooltipRes, className, recordName, useSubdock, bFront)
	function fCreate()
		DesktopManager.createDockShortcut(iconNormal, iconPressed, Interface.getString(sTooltipRes), className, recordName, useSubdock, bFront);
	end
...
function createDockShortcut(iconNormal, iconPressed, tooltipText, className, recordName, useSubdock, bFront)
	...
	c.setData(tooltipText, className, recordName or "");
...
function addLibraryDockShortcut(sRecordType)
	function fCreate()
		DesktopManager.createLibraryDockShortcut(sRecordType);
	end
...
function createLibraryDockShortcut(sRecordType)
	...
	local c = m_wShortcuts.shortcutbar.subwindow.createControl("desktop_dockitem", sRecordType);
...
function initializeSidebar()
	...
	aSidebarOrder = {};
	if not CampaignRegistry.sidebar then
...
function resetSidebar(sMode)
	...
	elseif sMode == "" then
		CampaignRegistry.sidebar = "";
	else
		CampaignRegistry.sidebar = aDefaultSidebarState[sMode] or "";
	end
	
	DesktopManager.initializeSidebar();

	local wSidebar = Interface.findWindow("sidebar", "");
	if wSidebar then
		for _,w in ipairs(wSidebar.list.getWindows()) do
			w.synchState();
		end
	end
end
...
function onSidebarOptionChanged(sRecordType, nValue)
	...
	DesktopManager.setSidebarButtonState(sRecordType, (nValue == 1));
end
...
function setSidebarButtonState(sRecordType, bState)
	local bCurrentState = DesktopManager.getSidebarButtonState(sRecordType);
...
function addTokenPackSets(aTokenPacksSetsValue)
	...
	for _,v in ipairs(aTokenPacksSetsValue) do
		DesktopManager.addTokenPackSet(v.name, v.modules);
	end
end
