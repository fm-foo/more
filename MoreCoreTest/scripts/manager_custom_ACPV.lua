...
--	Debug.console("Preflightcheck: ", rRoll.sDesc);
  ...
  local sDice, sOp, sSave1 = sDicePattern:match("([^%s]+)([<>])([+-]?%d+)");
--	Debug.chat("sDice, sOp, sSave1: ", sDice, sOp, sSave1);
  local nSave1 = tonumber(sSave1);
  	local sDesc1, nMod = ModifierStack.getStack(true);  
	local nSave = math.floor((nSave1)+(nMod));
	--Debug.chat("nSave: ", nSave1);
...
function createChatMessage(rSource, rRoll)  
  local rMessage = ActionsManager.createActionMessage(rSource, rRoll);
    ...
    rMessage.text = rMessage.text .. "\n[Skill Check - Target under" .. rRoll.aSave .. "]\n[" .. rRoll.aSaveResult .. "] " .. rRoll.aTotal;
