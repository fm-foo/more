...
-- MoreCore v0.60 register "bspell"
function onInit()
  CustomDiceManager.add_roll_type("bspell", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /bspell [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "bspell", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
