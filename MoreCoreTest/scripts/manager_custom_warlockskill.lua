...
-- MoreCore v0.60 register "wskill"
function onInit()
  CustomDiceManager.add_roll_type("wskill", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /wskill [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "wskill", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
