...
--Debug.console("performAction: ", draginfo, rActor, sParams);
  local sDamage, sDesc = string.match(sParams, "([^%s]+)%s*(.*)");
...
--Debug.chat("onLanded35: ", rSource, rTarget, rRoll);
...
--	Debug.chat("sParams55: ", sParams);
  local nStart, nEnd, sDicePattern, sDescriptionParam = string.find(sParams, "([^%s]+)%s*(.*)");
  rRoll.sDesc = sDescriptionParam;
--	Debug.chat("sDicePattern58: ", sDicePattern);

  -- Now we check that we have a properly formatted parameter, or we set the sDesc for the roll with a message.
  if not sDicePattern:match("(%d+)d([%dF]*)s(%d+)") then
    rRoll.sDesc = "Parameters not in correct format. Should be in the format of \"#d#s#\" ";
    return rRoll;
  end

  local sDice, sDesc = string.match(sParams, "([^%s]+)%s*(.*)");
  local sDice, sSuccess = string.match(sDice, "([^s]+)s(.*)");
--	Debug.chat("sDice, sSuccess68: ", sDice, sSuccess);
...
--		Debug.chat("ipairs(aDice): ", k, v);
 ...
 --      Debug.chat("aFinalDice100: ", aFinalDice);
  end
  ...
  if #aNonStandardResults > 0 then
	...
	local rRoll = { sType = sCmd, sDesc = sDesc, aDice = aFinalDice, nMod = nMod, nSuccessTarget = tonumber(sSuccess) };
--	Debug.chat("rRoll114: ", rRoll);
	return(rRoll);
...
--	Debug.chat("rRoll.aDice130: ", v, rRoll.aDice, rRoll);
	if v.result < tonumber(rRoll.nSuccessTarget) then
		v.value = 0;
		nTotal = nTotal;
	else
		v.value = 1;
...
--	Debug.chat("rRoll152: ", rRoll);
  if rTarget ~= nil and rTarget.sCTNode ~= nil then
--	Debug.chat("DR!!!!");
--    myTargetArmor = tonumber(DB.getValue(rTarget.sCTNode .. ".four"));
--	local nMinDamage = rRoll.aTotal - myTargetArmor;
--	Debug.chat("nMinDamage!!!! ", myTargetArmor);
