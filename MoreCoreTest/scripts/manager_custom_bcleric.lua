...
-- MoreCore v0.60 register "bcleric"
function onInit()
  CustomDiceManager.add_roll_type("bcleric", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /bcleric [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "bcleric", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
