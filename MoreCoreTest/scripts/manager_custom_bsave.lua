...
-- MoreCore v0.60 register "bsave"
function onInit()
  CustomDiceManager.add_roll_type("bsave", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /bsave [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "bsave", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
