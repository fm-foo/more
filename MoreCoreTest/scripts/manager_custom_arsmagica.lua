...
function initLastDice(aDice)
	...
	for i , v in ipairs (aDice) do
		aSavedDice[i] = { type=v.type, result=0, exploded=false };
	end
	return aSavedDice;
end
...
function onLanded(rSource, rTarget, rRoll)
	...
	for i , v in ipairs (aSavedDice) do
		...
		elseif w.result == 10 then
			if v.exploded == true then
				...
				nMyTotal = 10 * 2^multi;
				multi = 0;
				Debug.console("Total exploded 10:", nMyTotal);
				v.exploded = false;
			else
				if not sDiceLanded then
					sDiceLanded = w.result;
				...
				multi = 0;
				v.exploded = false;
				Debug.console("Total botch:", nMyTotal);
			end
		else
			if v.exploded == true then
				if not sDiceLanded then
				...
				Debug.console("multi:", multi);
				nMyTotal = w.result * 2^multi;
				multi = 0;
				Debug.console("w.result:", w.result);
				Debug.console("Total:", nMyTotal);
				v.exploded = false;
			else
				if not sDiceLanded then
					sDiceLanded = w.result;
	...
	rMessage.type = sCmd;
	rMessage.text = rMessage.text .. "\r[Dice] " .. sDiceLanded .. "\r[Result] " .. nMyTotal;
	rMessage.nSuccessValue = nMyTotal;
...
function onDiceTotal( messagedata )
	...
	local sMyTotal = string.match(messagedata.text, "t]%s(.*)");
