...
function getDiceResults(rRoll, rTarget)
  ...
  local nThac0 = tonumber(rRoll.nThac0);
	...
	if nTotal == 1 then
			...
			sHitResult = "Automatic Miss";
		elseif nTotal == 20 then
			Debug.console("Dice 1: ", nTotal);
			sHitResult = "Automatic Hit";
		elseif rTarget ~= nil then
			if nThac0 - nTotal <= tonumber(DB.getValue(rTarget.sCTNode .. ".defence")) then
				sHitResult = "Hit!";
			elseif nThac0 - nTotal > tonumber(DB.getValue(rTarget.sCTNode .. ".defence")) then
				sHitResult = "Miss!";
			end 
		elseif nTotal < nThac0 - 9 then
			Debug.console("Dice Miss: ", nTotal);
			sHitResult = "Miss";
		elseif nTotal >= nThac0 - 9 then
			Debug.console("Dice Hit: ", nTotal);
			nSuccessAC = nThac0 - nTotal;
