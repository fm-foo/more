...
OOB_MSGTYPE_APPLYDAMAGE = "applygsdamage";
...
--    Debug.chat("myTargetArmor: ", myTargetArmor, " rRoll.aTotal: ", rRoll.aTotal );
...
function applyDamage(rTarget, nDamage)
  ...
  if rTarget ~= nil and rTarget.sCTNode ~= nil then
    ...
    Debug.console("myTargetArmor: ", myTargetArmor);
	rTarget.myTargetArmor = myTargetArmor;
	
	local nMinDamage = nDamage - myTargetArmor;
	if nMinDamage < 0 then nMinDamage = 0; end
    Debug.console("nMinDamage: ", nMinDamage);
	
    local myTargetNewWounds = myTargetWounds + nMinDamage;
