...
-- Debug.console("performAction: ", draginfo, rActor, sParams);
  local sDice, sMult, sDesc = string.match(sParams, "([^%s]+)x(%d)%s*(.*)");
...
--- rRoll.sUser = User.getUsername();
  ...
  local nStart, nEnd, sDicePattern, sMult, sDescriptionParam = string.find(sParams, "([^%s]+)x(%d)%s*(.*)");
  ...
  local aFinalDice = {};
  local nMult = tonumber(sMult);
--  Debug.chat("nMult: ", nMult);
  ...
  if #aNonStandardResults > 0 then
	...
	local rRoll = { sType = sCmd, sDesc = sDesc, aDice = aFinalDice, nMod = nMod, nMult = nMult };
	return(rRoll);
...
--	Debug.chat("v: ", v.result);
	nTotal = nTotal + v.result;
	end
--	Debug.chat("v 1: ", nTotal);
--	nTotal = nTotal + rRoll.nMod;
--	Debug.chat("v 2: ", nTotal);

  rRoll.aTotal = (nTotal * rRoll.nMult) + rRoll.nMod;
--	Debug.chat("v 3: ", rRoll.aTotal);
--	nTotal = nTotal + rRoll.nMod;
--  Debug.chat("rRoll.aTotal: ", rRoll.aTotal);
  return rRoll;
end
...
--  Debug.console("onDiceTotal: ", messagedata);
  number = tonumber(string.match(messagedata.text, "%s%d+"));
--  Debug.console("number: ", number);
  return true, number;
end
