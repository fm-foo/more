...
--	Debug.console("Preflightcheck: ", rRoll.sDesc);
  if not sDicePattern:match("([^%s]+)x([^%s]+)") then
    rRoll.sDesc = "Parameters not in correct format. Should be in the format of \"#d#+#x#+#\" ";
    return rRoll;
  end

  local sDice, sSave1 = sDicePattern:match("([^%s]+)x(%d+)");
--	Debug.chat("sDice, sSave1: ", sDice, sSave1);
	local sDesc1, nMod = ModifierStack.getStack(true);  
	local nSave1 = tonumber(sSave1);

--	Quarter and half checks	
	if Input.isControlPressed() then
		nSave1 = math.ceil(nSave1/2);
	elseif Input.isAltPressed() then
		nSave1 = math.ceil(nSave1/4);
	end
--	Quarter and half checks	

	local nSave = nSave1+nMod;
