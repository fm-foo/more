...
--- rRoll.sUser = User.getUsername();
  ...
  local nStart, nEnd, sDicePattern, sDescriptionParam, nSave = string.find(sParams, "([^%s]+)%s*(.*)x(%d+)");
...
--  local nSave = 10;
  ...
  local rRoll = { sType = sCmd, sDesc = sDesc, aDice = aFinalDice, nMod = nMod, nSave = nSave };
...
nTotal = 0;
  ...
  local save = tonumber(rRoll.nSave);
	...
	Debug.console("mySave: ", save);
	if nTotal < save then
		Debug.console("Failed: ", nTotal);
		sChallegeResult = "[Challenge Roll Failure] ";
	elseif nTotal >= save then
			Debug.console("Success: ", nTotal);
			sChallegeResult = "[Challenge Roll Success] ";
		end

	Debug.console("sChallegeResult: ", sChallegeResult);

  rRoll.aTotal = nTotal;
  rRoll.sChallegeResult = sChallegeResult;
  return rRoll;
end
---
...
function createChatMessage(rSource, rRoll)  
  local rMessage = ActionsManager.createActionMessage(rSource, rRoll);
    ...
    rMessage.text = rMessage.text .. "\n" .. rRoll.sChallegeResult .. rRoll.aTotal;
