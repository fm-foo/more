...
-- MoreCore v0.60 register "wranged"
function onInit()
  CustomDiceManager.add_roll_type("wranged", performAction, onLanded, true, "all");
end
...
Debug.console("performAction: ", draginfo, rActor, sParams);
  ...
  if sDice == nil then
    ChatManager.SystemMessage("Usage: /wranged [dice+modifier] [description]");
    return;
  else
    sDice = sDice;
  end
  ...
  local rRoll = { sType = "wranged", sDesc = sDesc, aDice = aFinalDice, nMod = nMod };
